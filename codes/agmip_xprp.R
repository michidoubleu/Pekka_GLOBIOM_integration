...
# 1. Load Data
df_xprp <- m["Price_compare"]$records
colnames(df_xprp) <- c("region","item","ssp","scen1","For_Scen","year","value")
...
forest_xprp_tidy <- df_xprp_clean %>% filter(value>0) %>%
  mutate(
    Model = "GLOBIOM",
...
saveRDS(forest_xprp_tidy, "./output/processed_forest_xprp.rds")